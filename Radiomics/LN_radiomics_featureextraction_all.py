...
#    DFSevents = Tumor_list['DFSevents'].tolist()
    ...
    for i in tqdm(range(len(List_Num))):
        ...
        if os.path.exists(img_path+'/'+str(dcm_File)):
            ...
            mask_path = mask_root+'/'+str(dcm_File)+'/'+str(dcm_File)+' L.nii'
            if ~os.path.exists(mask_path):
                mask_path = mask_root+'/'+str(dcm_File)+'/'+str(dcm_File)+' L.nii.gz'

        else:
            roi_path = '../../TestData/DICOM/'+str(dcm_File)
            mask_path = '../../TestData/ROI/'+str(dcm_File)+' L.nii.gz'
            if ~os.path.exists(mask_path):
                mask_path = '../../TestData/ROI'+'/'+str(dcm_File)+' L.nii'
        try:
...
#            features['DFSevents'] = DFSevents[i]
    ...
    df.to_csv('../../LN_New_list.csv',index=False,sep=',')
