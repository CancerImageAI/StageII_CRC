...
def augment(roi, label, ifflip = True, ifrotate=True, ifswap = True):
    if ifrotate:
        ...
        label = rotate(label,angle1,axes=(1,2),reshape=False)
    if ifswap:
...
class DataGenerator(Dataset):
    ...
    def __getitem__(self,idx):  
        ...
        if self.phase == 'train':
            ...
            Label = (Label>0)[np.newaxis,...].astype('float32')
...
#        loss_output = loss(m(output), target)
#        print(loss_output)
...
if __name__ == '__main__':
    ...
    img_path = '../Tumor_crop'
    dataset = DataGenerator(img_path, phase='train')
    data_loader = DataLoader(dataset, batch_size = 6,shuffle = True, num_workers = 0, pin_memory=True)
    save_freq = 20
    epochs = 200
    lr = 0.001
    save_dir = './model'
