# -*- coding: utf-8 -*-
"""
Created on Tue Nov 19 09:32:15 2019
...
def Seed_Transposition(seed_pos, increments):
    new_seed = []
    for i in increments:
        for j in ['x','y']:
            if j=='x':
                new_seed.append([seed_pos[0]+i, seed_pos[1], seed_pos[2]])
            elif j=='y':
                new_seed.append([seed_pos[0], seed_pos[1]+i, seed_pos[2]])
    return new_seed
...
if __name__ == '__main__':
    ...
    data_path = '../DICOM'
    mask_path = '../ROI'
    OS_save_path = '../Train_Crop/OS_Crop'
    DFS_save_path = '../Train_Crop/DFS_Crop'
...
#                np.save(os.path.join(save_path,str(Tumor_num)+'_label.npy'), ROI_label)
                if OS_Class==1:
                    increments = [-2,-1,1,2,3]
                    new_seed = Seed_Transposition(seed_pos, increments)
    ...
    df.to_csv('./train_OS_List.csv',index=False,sep=',')
    df = DF(DFS_List).fillna('0')
    df.to_csv('./train_DFS_List.csv',index=False,sep=',')
